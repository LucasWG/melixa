...
function sendEmailsJustificativa() {
	...
	var mapeamentoDestinatariosDefault =
		'genderson.reseende@mercadolivre.com, marcelo.esteves@mercadolivre.com, lucas.cdsouza@mercadolivre.com'
	...
	var ccoDefault = 'johnny.msilva@mercadolivre.com, lucastav.campos@mercadolivre.com'
	...
	for (var i = 1; i < data.length; i++) {
		...
		if (foiEnviado === false || foiEnviado === 'FALSE' || foiEnviado === '') {
	...
	origens.forEach(function (origem) {
		...
		if (mapeamentoDestinatarios.hasOwnProperty(origem)) {
			...
			gruposPorOrigem[origem].forEach(function (item) {
				var row = item.dados
				var dataFormatada = Utilities.formatDate(new Date(row[3]), 'GMT-3', 'dd/MM/yyyy')
				var custoBRLFormatado = parseFloat(row[6]).toFixed(2).replace('.', ',')

				corpoHtml += '<tr>'
				corpoHtml += "<td style='padding: 8px; text-align: center;'>" + row[1] + '</td>'
				corpoHtml +=
					"<td style='padding: 8px; text-align: center;'>" + dataFormatada + '</td>'
				corpoHtml += "<td style='padding: 8px; text-align: center;'>" + row[4] + '</td>'
				corpoHtml += "<td style='padding: 8px; text-align: center;'>" + row[5] + '</td>'
				corpoHtml +=
			...
			GmailApp.sendEmail(destinatario, 'Pacote(s) On Way SMG2 - ' + origem, '', {
				htmlBody: corpoHtml,
				bcc: ccoDefault
			})
		}
